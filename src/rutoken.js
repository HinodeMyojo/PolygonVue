...
import rutoken from './scripts'

var plugin

function checkVersion(lastVersion) {
  if (plugin.version.toString() < lastVersion) console.log('download last version: ' + lastVersion)
  else console.log('you have last version')
}
...
function getLastRtPluginVersion(callback) {
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState == 4 && xhr.status == 200) {
      var lastPluginVersion = this.response
        .split('Version: v.')[1]
        .split('Release')[0]
        .replace(/\s+/g, '')
...
export const sign = function (hashvalue) {
  var rutokenHandle, certHandle
  // Получение текста для подписи
  var textToSign = hashvalue
